...
STATS_CACHE = {}  # Stores { (where_sql, params_tuple): (data_dict, timestamp) }
STATS_CACHE_TTL = 60  # 1 minute TTL to keep stats responsive but cached
...
def build_contact_filters(
    ...
    if search_query:
        ...
        # Optimize if query is digit/phone or has @ for email
        if clean_sq.isdigit() or (clean_sq.startswith('+') and clean_sq[1:].isdigit()) or (len(clean_sq) >= 7 and re.match(r'^[\d+\-\s()]+$', clean_sq)):
            where_clauses.append(f"(phone_clean {like_op} {ph} OR phone {like_op} {ph})")
            params.extend([f"%{clean_sq}%", f"%{clean_sq}%"])
        elif '@' in clean_sq:
            where_clauses.append(f"email {like_op} {ph}")
            params.append(f"%{clean_sq}%")
        else:
            where_clauses.append(
                f"(name {like_op} {ph} OR city {like_op} {ph} OR category {like_op} {ph} OR source {like_op} {ph})"
            )
            search_pattern = f"%{clean_sq}%"
...
def init_tables():
    ...
    try:
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_contacts_scraped_at ON contacts(scraped_at DESC)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_contacts_quality_score ON contacts(quality_score DESC)")
...
/* Notification Toast */
        ...
        .export-btn-blue svg { stroke: var(--accent-blue); }
        .export-btn-blue span { color: var(--accent-blue); font-weight: 700; font-size: 13px; }
        .export-btn-blue:hover { background: rgba(59,130,246,0.15); border-color: var(--accent-blue); }
        .export-btn-amber {
        ...
        .export-btn-amber svg { stroke: var(--accent-amber); }
        .export-btn-amber span { color: var(--accent-amber); font-weight: 700; font-size: 13px; }
        .export-btn-amber:hover { background: rgba(245,158,11,0.15); border-color: var(--accent-amber); }
    ...
    <main class="main-view">
        <div class="header-row">
            <div class="page-title">
                ...
                {% else %}
                <h2>Finance{% if selected_category %}<span style="color: var(--accent-blue);"> / {{ selected_category }}s</span>{% endif %}</h2>
                <p>Manage scraped financial leads</p>
                {% endif %}
            </div>
                ...
                <div class="table-section">
                    <div class="table-header">
                        ...
                        <div class="table-actions">
                            <span class="record-count">{{ contacts|length }} of {{ "{:,}".format(s.filtered_total) }} records</span>
                            <button onclick="exportData('csv', this)" class="export-btn-green" title="Export filtered as CSV">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                                <span>CSV</span>
                            </button>
                            <button onclick="exportData('xlsx', this)" class="export-btn-blue" title="Export filtered as Excel">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                                <span>Excel</span>
                            </button>
                            <button onclick="exportData('json', this)" class="export-btn-amber" title="Export filtered as JSON">
...
window.updatePaginationUI = function(data) {
        ...
        window.downloadBlob = async function(url, filename, btn, originalHTML) {
            try {
                const res = await fetch(url, { method: 'GET', credentials: 'same-origin' });
                if (!res.ok) {
                    const text = await res.text();
                    throw new Error('Export failed: ' + res.status + ' ' + text.substring(0, 200));
                }
        ...
        window.exportData = async function(fmt, btn) {
            const q = document.getElementById('t-cat')?.value || "";
            const city = document.getElementById('t-city')?.value || "";
            ...
            await window.downloadBlob(url, 'export_' + ts + '.' + ext, btn, originalHTML);
        };

        window.exportAllData = async function(fmt, btn) {
            const originalHTML = btn ? btn.innerHTML : '';
            if (btn) {
                btn.disabled = true;
                btn.innerHTML = '<div class="spinner-sm"></div>';
            ...
            const ext = fmt === 'xlsx' ? 'xlsx' : fmt;
            const ts = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14);
            await window.downloadBlob(url, 'bulk_export_' + ts + '.' + ext, btn, originalHTML);
        };

        window.exportCategory = async function(category, fmt, btn) {
            const originalHTML = btn ? btn.innerHTML : '';
            if (btn) {
                btn.disabled = true;
                btn.innerHTML = '<div class="spinner-sm"></div>';
            ...
            await window.downloadBlob(url, safeCat + '_' + ts + '.' + ext, btn, originalHTML);
        };
...
def render_dashboard_portal(is_school_dashboard=False):
    try:
        ...
        # High-performance caching for stats and aggregations to prevent DB throttling
        ...
        def run_clean():
            try:
                ...
                for row in rows:
                    ...
                    # Check if anything changed to save it all
